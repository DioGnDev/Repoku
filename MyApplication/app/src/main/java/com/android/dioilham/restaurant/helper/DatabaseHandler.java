...
//    private static final String KEY_JENIS_CART = "jenis_cart";
    ...
    private static final String CREATE_CART = "CREATE TABLE "
            + TABEL_CART + "(" + KEY_ID + " INTEGER PRIMARY KEY,"
            + KEY_KODE_CART + " TEXT,"
            + KEY_NAMA_CART + " TEXT,"
            + KEY_HARGA_CART + " TEXT,"
            + KEY_KETERANGAN_CART + " TEXT,"
            + KEY_QTY_CART + " TEXT,"
            + KEY_SUBTOTAL_CART + " TEXT)";
...
//            kode.put(KEY_rec_url, cur.getString(1));
    ...
    // -------------------------------- begin Add To Cart
    public void addToCart(String kode, String nama, String harga, String keterangan,
    ...
    public void hapusItemCart(String kode) {
        SQLiteDatabase db = this.getWritableDatabase();
        String[] args = new String[]{kode};
        db.delete(TABEL_CART, "kode_cart=?", args);
        db.close();
    }
    ...
    /*public void updateItemCartDetail(String kode, String qty) {
        String[] args = new String[]{kode};
        SQLiteDatabase db = this.getReadableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(KEY_QTY_CART, qty);
        db.update(TABEL_CART, cv, "kode_cart=?", args);
    }*/

    public ItemCart selectCart(String kode) {
        ItemCart data = new ItemCart();
        String countQuery = "SELECT * FROM " + TABEL_CART + " where kode_cart='"+ kode +"'";
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cur = db.rawQuery(countQuery, null);
        if (cur.moveToFirst()) {
            data.setKodeCart(cur.getString(1));
            data.setNamaCart(cur.getString(2));
            data.setHargaCart(cur.getString(3));
//            data.setJenisCart(cur.getString(4));
            data.setKetCart(cur.getString(4));
            data.setQtyCart(cur.getString(5));
            data.setSubtotalCart(cur.getString(6));
        }
        cur.close();
        db.close();
        return data;
...
//                dataCart.setJenisCart(cur.getString(4));
                dataCart.setKetCart(cur.getString(4));
                dataCart.setQtyCart(cur.getString(5));
                dataCart.setSubtotalCart(cur.getString(6));
